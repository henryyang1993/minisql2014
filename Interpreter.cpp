...
CString Interpreter(CString statement)
{
	...
	temp=SQL.Mid(start,end-start); //第一个词
	start=end+1; //从第二个词start
	//若无输入，打印出错信息
	if(temp.IsEmpty())
	{
...
CString read_input()
{
	...
	while(!finish)
	{
		...
		temp=str;
		SQL=SQL+" "+temp; //?
		if(SQL.GetAt(SQL.GetLength()-1)==';') //计数从0开始
		{
...
CString create_database(CString SQL,int start)
{
	...
	temp=SQL.Mid(start,end-start); //数据库名
...
/////////////////////////////////////////////////////////////////////////////////////////////
//验证drop table语句是否有效
CString drop_table(CString SQL,int start)  //drop table 表名 ;
{
	CString temp;
	int end;
	//获取第三个单词
...
/////////////////////////////////////////////////////////////////////////////////////////////
//验证drop index语句是否有效
CString drop_index(CString SQL,int start)  //drop index 索引名 ;
{
	CString temp;
	int end;
	//获取第三个单词
...
/////////////////////////////////////////////////////////////////////////////////////////////
//验证insert语句是否有效
CString insert_clause(CString SQL,int start)  //insert into 表名 values ( 值1 , 值2 , … , 值n ) ;
{
	CString temp,sql;
	int end;
	//获取第二个单词
...
{
	CString temp;                      //insert into 表名 values ( 值1 , 值2 , … , 值n ) ;
	CString T;
	int end;
	//获取(
	while(SQL.GetAt(start)==' ')
